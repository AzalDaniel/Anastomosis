...
Two pieces of work. First an audit of everything 0.7.0 shipped — driving the
real CLI and the real GUI rather than reading them, with each finding raised as
an issue and fixed in its own pull request. Then a reduction pass: thirteen
slices, each merging things that did one job in several voices, each measured
against a byte-for-byte snapshot of what the previous commit wrote.

`src/` fell from 52,070 lines to 42,312. Almost all of that was prose the code
did not need: docstrings narrating bug history, comments restating the line
below. Executable code fell 308 lines — far short of the plan, because the
per-slice estimates had counted prose the first slice already removed. The
consolidations are real; they simply bought fewer lines than the arithmetic
assumed. Tests gained 1,670 lines of code, because every consolidation left a
guard behind.

Nothing a user types changed. Every command, flag and default is byte-identical,
and the C-CDA digest over 6,144 generated documents is unmoved.

### Added

- **The rulebook and the safety net.** `docs/RULES.md`, `docs/AUDIT_LEDGER.md`,
  `CLAUDE.md` and six skills, plus the tools that hold them: a snapshot of every
  deliverable, a prose ratchet, a floor under the regression-guard count, and a
  clock seam that makes two runs byte-comparable. (#415)
- **A conservation ledger for C-CDA ingest**, and a 6,144-document generated
  corpus to run it against — every element the source offered, and what became
  of it. (#309)
- **A run names the inputs it was prepared under** and refuses when any of them
  changed underfoot. (#348)
- **What rendered is what was reviewed, and it can be named.** The reviewed
  route and the run's gates ride in the manifest; an executor refuses without
  them. (#350)
- **A `--bundle` `DocumentReference`** for a document sitting right beside the
  chart it belongs to. (#382)
- The guided session opens on the vessel mark rather than a pipe character.

### Changed

- **Teach-a-format and teach-a-layout are one learn capability.** Both flows
  were already analyze → confirm → emit, so they now share one command object,
  one confirm gate and one trust write. `anast pack init` and `anast source
  init` are unchanged — this is an internal consolidation, and the single
  `anast learn` front door the plan called for is not built yet.
- **Both Teach modes run on one scaffold in the browser**, driven by a step
  descriptor the backend returns.
- **One FHIR field table** replaces three hand-written mappers.
- **One deliverer writes both file trees** — the archive and the bundle differ
  in grouping, not in operation.
- **One walk discovers template packs and destination packs.**
- **One seam attaches a destination**, where the CLI and the GUI each had their
  own.
- **QA and the verification ladder go flat**: checks are functions in one file,
  and the L0–L6 ladder is a table rather than a class per level.
- **The command layer left the primitives package**, so `core/` is leaf-only.
- **The shared primitives have one implementation each** — one hasher, one
  atomic write, one date parser, one identity rule.
- **One HL7 `II` rule in one place**, instead of four hand-written copies that
  had each been fixed separately and drifted apart. (#320, #393, #404, #412)
- **The prose sweep**: the code says less and means the same.
- **The greeting mark arrives and then stops moving.** It reaches the menu in
  0.86 s rather than 8.61 s; the settled mark is character- and colour-identical
  to before. (#428)
- **Which visits become charts is a run's choice**, not the product's opinion.
  The rules that skipped a visit are now options a person can see and switch.
  (#288)
- **The browser pack's selector slots are read off the schema** that declares
  them, rather than three hand-typed tuples.
- **A trust hash gates execution, not editing** (rule 111), and third-party pack
  code no longer runs with the desktop user's authority. (#350)
- **`anast upload` refuses an unverified bundle.** A deliberate behaviour
  change. (#350)
- **The draft pack's markup lives in template files**, and a half-written draft
  is no longer left behind as if it were one.
- The guided session stops telling you every destination is ready to receive
  charts when only one has a filing assistant.
- A selector under a name the loader does not know is refused, loudly.
- A layout or an export format is shown by the name its author typed. (#164)
- Every command's help says what that command does, and names a next step that
  exists. (#196, #197, #201, #203)
- `anast info` says what is installed; `anast doctor` is the command that tries
  things. (#190)
- The Uploads counters are named for what they count: charts in the record, and
  PDFs in the folder. (#214)
- Charts already in a folder answer the question being asked. (#189)
- The complexity ratchet no longer punishes a deletion.

### Fixed

**Identity and conservation.** These are the ones that could put the wrong thing
in a chart.

- **Two patients whose medical-record numbers collided were merged into one
  chart.** Reading half an identifier made two people look like one. (#393,
  #404)
- **One patient was several charts, and only one of them arrived.** (#375)
- **A visit named by two documents became two charts**, where the source stated
  one id. (#378, #393)
- **A uuid4 minted at parse time rode into every delivered bundle**, so two
  loads of one document wrote two different bundles. Each fact now derives its
  id from what the source stated. (#405)
- **An entry under prose was preserved by nothing**, and an id-less organizer
  component doubled on export and stayed doubled. (#364, #365, #366)
- **`--ccda` delivered a patient's chart with none of their documents on it.**
  (#373)
- **An upload manifest came back empty** for a patient whose whole chart is
  attachments. (#374)
- **A chart with no encounters was never verified.** (#383)
- **A positive verdict has to be backed by the thing it claims**: a section that
  never looked does not get to deny. (#314)
- **A vital carried by the rendered summary was graded as on no chart at all.**
  (#239, #392)
- **A bundle's QA report left out the verdict on the one page the bundle is
  for**, and a QA context read back the first document it ever saw rather than
  the one it was asked about. (#398, #399)
- Two record-level lists never reached a FHIR bundle at all.
- Two files whose names sanitize alike could take one delivered slot; two charts
  can never land on one file. (#186)
...
**Reading real vendor exports.**

- **A `.ccd` export was never read** — the directory walk matched one extension.
  (#384)
- **A vendor's `value="0"` date sentinel aborted the whole export.** (#385)
- **An encounter with no type and no note was exported by nothing.** (#388)
- **A migrated C-CDA chart says who wrote it again.** (#312)
- **A scanned chart is still a chart**: an Unstructured Document carries one.
  (#313)
- A document with an empty mime type came back as a different type.
- The rail read "0 warn, 0 fail" over a chart that had abbreviated thirteen
  facts. (#271, #297)
- The corpus generator wrote four shapes C-CDA R2.1 does not permit. (#327)
- The layout you taught it is now the layout you can run.
- A destination pack can describe the filing dialog it actually meets.

**The window and the terminal.**

- A run that is happening now says so. (#198)
- The filing calendar is a table you read, not 42 buttons you cannot press.
  (#198)
- Escape closes what Escape opened. (#198)
- Four attributes that named nothing — a `<label for>` pointing at a `<div>`,
  among them. (#198)
- An upload that dies says so before it dies. (#117)
- A machine that cannot render charts says so once, with the remedy. (#202)
- The activity shortcut fired on the wrong keys, and the Uploads search
  truncated twice while mentioning neither cut. (#214)
- The error banner never left. (#214)

### Packaging

- **One build, two doors**: the Windows job also emits an MSIX for the Microsoft
  Store, staged from the same build rather than a second one. (#292)
- **The Windows installer stopped being rebuilt on every source merge** — an
  hour of the scarcest runner class, for an artifact nobody downloaded.
- **The shipped SBOMs name a version**, and the installer's is generated after
  the smoke gate rather than before: an SBOM for an installer that has not been
  shown to install is a statement about nothing. (#142)
- A build-backend bump nobody could accept no longer blocks every other pip
  update. (#389)
- The installed-footprint measurement can no longer fail a release. (#142)
- **A version bump read as thirty-eight changed deliverables.** The snapshot
  normalised the version out of JSON but not out of archive HTML or bundle
  `README.txt`; it is now a token in those too, so a stamp that disappears still
  fails.
