...
3. An attachment stem matching more than one file in an export is dropped from the index rather than resolved to an arbitrary match: an unresolved attachment is recoverable, a wrongly-guessed one is not. (`src/anastomosis/sources/pf_tebra/loader.py:270`)
