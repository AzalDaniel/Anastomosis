...
* **CLI/TUI**: same register, same state vocabulary — but NOT the same
  ...
  cannot drift apart), with the greeting beside it. It arrives from the trunk
  outward, then perfuses from the anastomosis — the junction the product is
  named for — and keeps perfusing until somebody presses a key. A console that
  cannot encode braille gets an ASCII ramp of the same shape, the same fallback
  the status glyphs already take, and the mark stands down entirely for three
  readers who are not watching it: a stream that is not a terminal, a window
  too narrow to hold the mark and a legible column of text, and anyone who set
  `NO_COLOR`.

  **The one amendment to the rule above, and its price.** The mark carries a
  colour ramp — `core/vesselmark.MARK_STOPS` — and it is the only absolute
  colour this program emits. The rule it amends is not repealed: a terminal's
  background still belongs to the person running it, and everything else on
  the CLI surface still names colours the terminal resolves. What changed is
  that a *decorative* object is governed by WCAG 1.4.11 (3 : 1) rather than
  1.4.3 (4.5 : 1), and that looser floor leaves a window — relative luminance
  0.175 to 0.242 — where a single ramp measurably clears 3 : 1 on a dark ground
  **and** a light one at once. Six stops fit inside it at the brand's own hue
  (OKLCh H = 30, the icon's measured 29.7), and they measure 3.05 – 3.38 : 1
  across nine real terminal grounds.

  The icon's own palette could not be used and this is why: it was sampled for
  oxblood on porcelain, so `TRUNK` `#701a14` measures **1.23 : 1** on One Dark
  and `CAP_RIM` `#5d130f` measures **1.04 : 1**. Even `--brand-bright` reaches
  only 2.68 : 1. The hue is kept; the lightness is re-derived. The gradient
  travels in *chroma* rather than lightness, because the safe window is only
  0.067 wide and because "more pigment" reads the same direction on both
  grounds where "brighter" inverts.

  Three limits make the amendment narrow rather than a hole in the rule:

  1. **Colour is never the only channel.** Glyph density and text weight still
     climb together, `dim` → plain → `bold`, and strip the colour and the mark
     is exactly what it was — asserted, frame by frame, in
     `test_the_mark_is_unchanged_with_the_colour_stripped`.
  2. **At sixteen colours the mark takes no colour at all.** Every stop
     quantises onto ANSI 9, which is `bright_red` — the refusal colour, which
     identity may not borrow. That rung, `NO_COLOR`, and any stream nobody is
     watching all draw exactly what shipped before this.
  3. **The licence is measured, not granted.** `terminal_colour_depth` decides
     the rung, and `test_every_mark_stop_clears_three_to_one_on_both_grounds`
     re-measures every stop against nine grounds and against the luminance
     window itself on every run. A stop that drifts out of the window fails the
     build even if it happens to pass on all nine.

  It stops for a keystroke, and otherwise after eight seconds. The cap is not
  timidity: driving the real `anast` on a pty at a sixty-second cap, the run
  never reached the menu — the mark was perfusing correctly and the question
  underneath it was waiting for it to finish. A greeting that holds the prompt
  is not alive, it is in the way. Motion that genuinely never stops needs the
  mark to live *under* the prompt rather than before it, which needs the
  self-echoing reader in #330; until that lands, eight seconds is where
  continuous honestly ends.
