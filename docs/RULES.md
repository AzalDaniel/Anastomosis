...
109. CodeQL runs on push, pull request and a schedule with the `security-extended` suite and no repo-wide exclusion; a `# codeql[...]` suppression sits alone on its line, carries a `PHI-BY-DESIGN` or `PHI-FREE-BY-CONSTRUCTION` rationale in the lines above it, and lives in a file `SECURITY.md`'s suppression policy names. `tests/unit/test_codeql_policy.py` pins all of it.
110. Every third-party GitHub Action is pinned to a full forty-character commit SHA, and a step that mutates alerts runs only after a push to `main`.
111. A trust hash gates execution, not editing: a pack whose hash changed is unavailable (22) because its `context.py` is code the renderer would run, while a learned mapping whose `source_trust.json` hash no longer matches still loads and warns by name, because `mapping.json` is data every load re-validates against the closed target and transform sets (30) and refining it by hand is what `anast source init` tells the operator to do.
