...
the provenance of every part of the codebase — including exactly how AI
assistance was used and what the author personally built.
...
## Authorship, AI assistance & provenance

**Author: Azal Daniel.** Anastomosis generalizes a private production system
by the same author that migrated a clinic's full EHI export with no
wrong-patient events. Nothing was copied from that system — every port is a
re-typed refactor, enforced by the PHI scanner and the scrub map in
`docs/PLAN.md`.

**AI assistance, disclosed and cited.** Anastomosis is developed with
substantial assistance from AI coding agents (Anthropic Claude-family
models) operating under the author's direction and review. The division of
labor: the author sets the problem, scope, architecture, clinical-domain
requirements, security posture, and acceptance criteria; reviews and
adjudicates every change; and owns every decision of record. Agent-produced
code must pass strict gates before merge — `ruff` (including bandit-S),
`mypy --strict`, the full test suite, the PHI scanner — plus a two-strand
adversarial review process codified in `.claude/skills/`. Source files carry
a one-line AI-assistance citation comment (the [CS50 academic-honesty
policy](https://cs50.harvard.edu/x/honesty/) asks for citation in code
comments; the convention here serves any reader). The same disclosure
appears in `paper/paper.md` and the README.

**Provenance table.**

| Component | Provenance |
|---|---|
| `src/anastomosis/**`, `tests/**`, `tools/**`, `packaging/**`, workflows | Authored for this repo by Azal Daniel with AI-agent assistance under the author's direction and review (cited per file) |
| Domain knowledge: PF chart forensics (`packs/practice_fusion_soap/RULES.md`), QA check contracts, sentinel semantics | Re-typed generalizations of the author's private predecessor system; no file copied (PHI rule) |
| `reconstruct/ccda_standard/vendor/CDA.xsl` (+ siblings) | Vendored from HL7, pinned and documented in `PINNED.md` |
| C-CDA test fixtures under `tests/fixtures/` marked Synthea | Generated by MITRE Synthea (synthetic patients) |
| PF/Tebra + Oracle V500 fixtures | Hand-built synthetic data per the public vendor data dictionaries (`feedface-` GUIDs, 555 phones, never-issued SSNs) |
| Rendering goldens | Generated artifacts, regenerated only via `tools/regen_goldens.py` with human-reviewed diffs |

**What the author personally built (CS50 framing).** The essence of this
work — the problem selection from lived practice-migration experience, the
five-stage architecture and canonical-model decision, the losslessness and
loud-failure invariants, the L0–L6 verification ladder design, the
destination evidence-or-refuse rule, the PHI-safe engineering process, the
review gates, and every design decision in this file — is the author's own.
AI agents amplified the build under that direction, and their use is cited
in the code itself, as the policy asks.
...
The GitHub release for an alpha is flagged "pre-release". Beta is reserved
for the CS50 submission cut.
